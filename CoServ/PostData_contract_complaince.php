...
class PostData_contract_complaince{
	...
	*/
          ...
          {      
            ...
            }elseif(strpos($this->formtitle, 'ROW Assessment') !== false) 
            {
              ...
              foreach($folders as $name)
              {
                   ...
                   $pathvalue .= "/$name";
                    if ($record->getResult() != '1'){    // creation failed
                        $exists = $cloudAPI->fileexists($pathvalue);
                         if($exists->getResult() != '1') //file does not exists
                         {
    ...
    /*This function is used to set the information that will give details in case of an error*/
      public function setErrorMessage($data)
      {
          ...
          $employee = $data['inspector'];
          $report_date = $data['work_date'];
          $project = $data['company'];
          $form_title = $data['form_title'];
          $this->record_info = 'Profile: '.$this->profile.' Form Title: '.$form_title.' Project: '.$project.' inspector: '. $employee. ' Record id : '.$this->record_id.' Work Date '.$report_date.'<br/>';
      }
  ...
  public function recursiveRemoval(&$array, $val)
  {
    ...
    */
    public function prepareMainform()
    {
        ...
        $main_form_keys = array("form_title","inspector","crew_no","company","work_date","gps_latitude","gps_longitude","weather_conditions");
									...
									<tr>
									  <td class='col inner' width='298' style='padding:2px 4px;padding: 10px;'>
										<table cellpadding='0' cellspacing='0' style='border-spacing:0;width:100%;display:block;'>
										  <tr>
											<td class='strong' style='padding:2px 4px;font-weight:800;width:30%;'>Inspector </td>
											<td style='padding:2px 4px;width:70%;'>{$mainform['inspector']}</td>
										  </tr>
										  <tr>
