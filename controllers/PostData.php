...
class PostData{
      ...
      private $headerlogo;
      private $email_trigger=false;
      private $emailalert_recipients;
      private $upload_status;
      private $upload_file;
      private $complete_upload_path;
      private $email_attachment = array();
      private $record_id;
      private $upload_date;
      private $profile;
      private $record_info;
	...
	*/
	...
	public function __get($name)
	{
      ...
      {
         //  $this->recipients = "alfiaa@thinkpowersolutions.com,vinod@thinkpowersolutions.com";
     ...
     */
     ...
     /*
       This function is used for sending an email when a file is uploaded
     */
    
      public function emailAlertOnUpload()
      {
            $email = new Email();
	        $email->to = $this->emailalert_recipients;
            $email->subject = "New file addition for ". $this->jsonarray['project'];
	        $email->body = "The file has been uploaded and the path is ".$this->complete_upload_path;
          
            $email->attachment=$this->email_attachment;
            $sent = $email->sendEmailapi();
           
	        if($sent!==200){
	          $email->SendErrorEmail($this->record_info.' '."Error in sending email alert on file upload. Error code $sent. Error Message is ".$email->failure_message.
                                     "<br/>The record id is: ".$this->record_id."<br/> Form Name is: ".$this->formtitle." <br/>Date of Upload is: ".$this->upload_date."<br/>");
	        }
      ...
      */
      public function createPDF()
      {
        try{
          ...
          // $lastfilename=$filename;
           ...
           // $filepath_main = "../tmp/$lastfilename.pdf";
            $ret = rename ($filepath_main, $filepath_new);
              
            $this->email_attachment[0]['Name'] = "$filename.pdf";
            $this->email_attachment[0]['Content'] = base64_encode(file_get_contents($filepath_new));
            $this->email_attachment[0]['ContentType'] = "application/pdf";
            ...
            try
          {
                  ...
                  if($record=='OK')
                  {
                      ...
                      $this->upload_date = date('m/d/Y');
                  }
                  $this->upload_status = $record;
                  $this->complete_upload_path = $upload_path;
                  $this->upload_file = "$filename.pdf";
          } 
          catch (Exception $e) 
          {
	        $btrace = $e->getTraceAsString();
            $email = new Email();
            $err_msg = $this->record_info.' Email Report file upload Caught exception: '. $e->getMessage(). "\n";
            $err_msg .='Trace: '. $btrace. "\n";
            $email->SendErrorEmail($err_msg);
          }
          }catch (Exception $e) 
          {
	        $btrace = $e->getTraceAsString();
      ...
      /* This function is used for setting the fileupload path variable. 
          ...
          {      
            ...
            elseif(strpos($this->formtitle, 'Line Construction Inspection') !== false)
            {
                ...
                elseif(strpos($this->subformtitle, 'Pole Jacking Assessment')!== false || strpos($this->subformtitle, 'Structure Erection Assessment')!== false)
                {
                 ...
                 elseif(strpos($this->subformtitle, 'Stringing / Pulling Assessment')!== false 
                        || strpos($this->subformtitle, 'Sagging Operation Assessment')!== false
                        || strpos($this->subformtitle, 'Compression Sleeves and Terminal Assessment')!== false
                        || strpos($this->subformtitle, 'Clipping Assessment')!== false
                        || strpos($this->subformtitle, 'Fiber Splicing')!== false
                        )
            ...
            }elseif(strpos($this->formtitle, 'ROW Assessment') !== false) 
            {
                ...
                } else
                {
                 ...
                 $folder_name ="Construction Inspections/$structure/Structure"; //default
            }
              ...
              foreach($folders as $name)
              {
                   ...
                   $pathvalue .= "/$name";
                    if ($record->getResult() != '1'){    // create folder failed
                         $exists = $cloudAPI->fileexists($pathvalue);
                         if($exists->getResult() != '1') // folder doesnt already exists
                         {
                  ...
                  $email = new Email();
                  $email->SendErrorEmail($this->record_info.' '."Upload folder is empty. File upload path ".$this->file_upload_path." Title ".$this->formtitle." ".$this->subformtitle);
              }
      ...
      **/
      ...
      public function setErrorMessage($data)
      {
          ...
          $this->record_info = 'Profile: '.$this->profile.' Form Title: '.$form_title.' Project: '.$project.' Employee: '. $employee. ' Record id : '.$this->record_id.' Report Date '.$report_date.'<br/>';
      }
  ...
  */
  public function recursiveRemoval(&$array, $val)
  {
   ...
   public function createBaseHeader()
   {
       ...
       $this->headerlogo = "<a href='http://thinkpowersolutions.com/' class='aligncenter' style='color:#ed1c24;clear:both;display:block;margin-left:auto;margin-right:auto;text-align:center;padding:1% 3%;'>
    ...
    */
    public function prepareMainform()
    {
        ...
        $main_form_keys = array("form_title","employee","project","report_date","client","manager","engineer","gps_latitude","gps_longitude","project_wo","contractor","weather_conditions");
									  ...
									  <td class='col inner' width='298' style='padding:2px 4px;padding: 10px;'>
										<table cellpadding='0' cellspacing='0' style='border-spacing:0;width:100%;display:block;'>
										  <tr>
											...
											<td style='padding:2px 4px;width:70%;'>{$mainform['employee']}</td>
										  </tr>
										  <tr>
